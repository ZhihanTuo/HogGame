...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while (score < goal) and (opponent_score < goal):
        if who == 0:
            num_rolls0 = strategy0(score, opponent_score)
            dice0 = select_dice(score, opponent_score)
            score += take_turn(num_rolls0, opponent_score, dice0)
            if swine_swap():
                score, opponent_score = opponent_score, score
        elif who == 1:
            num_rolls1 = strategy1(opponent_score, score)
            dice1 = select_dice(opponent_score, score)
            opponent_score += take_turn(num_rolls1, score, dice1)
            if swine_swap():
                score, opponent_score = opponent_score, score
        who = other(who)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    max = 0
    for i in range(10):
        temp = make_averaged(roll_dice)
        curr = temp(i + 1, dice)
        if curr > max:
            max = curr
            best = i + 1
        print(str(i + 1) + ' dice scores ' + str(round(curr, 2)) + ' on average')
    return best
    # END PROBLEM 6
...
def bacon_strategy(score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 7
    if BACON_MARGIN <= take_turn(0, opponent_score):
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 8
    roll_zero = take_turn(0, opponent_score)
    if roll_zero + score == opponent_score / 2: #beneficial swap
        return 0
    if roll_zero + score == opponent_score * 2: #loses points:
        return BASELINE_NUM_ROLLS
    if roll_zero + score != opponent_score * 2 and roll_zero + score != opponent_score / 2: #rolling 0 != swap, but rolling 0 >= BACON_MARGIN
        if roll_zero >= BACON_MARGIN:
            return 0
    return BASELINE_NUM_ROLLS
    # END PROBLEM 8
...
def final_strategy(score, opponent_score):
    ...
    This strategy takes less risk with a four-sided dice, 
    rolls zero if it can result in the opponent rolling a four-sided dice,
    utilizes the swap strategy when the opponent has 2.5x or more points,
    and always prevents rolling zero should it result in a deficit.
    """
    # BEGIN PROBLEM 9
    roll_zero = take_turn(0, opponent_score)
    ...
    if select_dice == four_sided:
        return 4
    if (score + roll_zero + opponent_score) % 7 == 0:
        return 0
    if score <= opponent_score / 2.5:
